        ...
        if (token) {
            ...
            if (resposta) {
                ...
                arrayTodosTextos.forEach(element => {
                    ...
                    if (element.pagina.hasOwnProperty("eventos")) {
                        element.pagina.eventos.forEach(ordem => {
...
async function salvarMudancas() {
    ...
    try {
        ...
        arrayTodasImagens.forEach(element => {
            ...
            if (element.pagina.hasOwnProperty("eventos") && element.src == imagem.getAttribute('src')) {
                id = element._id
            }
            if (imagem.nodeName == 'IMG') {
                arrayImagensParaSalvar.push({_id: id, ordem: ordemImagem, pagina: "eventos"})
            }
        })
